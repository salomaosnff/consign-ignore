...
Consign.prototype._checkIgnored = function(curr_path){
    var matched = false;

    for(var i in this._options.ignore){
        var ignore = this._options.ignore[i];
        if((ignore.test && ignore.test(curr_path)) || (ignore == curr_path)){
            matched = true;
            break;
        }
    }
...
Consign.prototype._setLocations = function(parent, entity, push) {
    ...
    if (fs.statSync(location).isDirectory()) {
        ...
        for (var e in dir) {
            var _e = dir[e];
            var _path = path.join(location, _e);

            if(this._checkIgnored(_e)){
                this._log([
                    '!', this._['Ignoring entity'], _e
                ], 'warn');
            } else if ('.' === _path.charAt(0)) {
                this._log([
                    '!', this._['Ignoring hidden entity'], path.join(location, _e)
                ], 'warn');
            } else {
                this._setLocations(location, _e, push);
            }
        }
